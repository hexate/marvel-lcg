...
class Controller:
    ...
    #
    @staticmethod
    def CanDecline(is_forced: 'bool|Literal["Forced_Action"]', effect_descriptors: Sequence[Any]) -> bool:
        """May the player answer this prompt with "no thanks" (input id 0)?

        An optional prompt, always. A forced one only when its single option asks for nothing, in
        which case declining is how it resolves. A forced prompt with a real choice in it has to be
        answered, or it was not forced.

        This used to be an inline assert, which meant a client offering a Cancel button on a forced
        prompt crashed the engine rather than being told no. See J8.
        """
        if not is_forced:
            return True
        return len(effect_descriptors) == 1 and effect_descriptors[0].target_num_range[0] == 0
    ...
    def ChoiceOne(self, effect_list: Sequence['Effect'], by_effect: 'Effect|None', message: 'Message2', priority: 'TimingPriority', is_forced: bool|Literal["Forced_Action"]) -> Tuple['Effect|None', bool]:
        ...
        while True:
            user_input = ''
            # Whether this answer was generated by skip/replay fallthrough rather than sent by a
            # client. Refusing a client's answer means asking the person again; refusing one the
            # engine produced would just produce the same answer forever.
            input_is_generated = False
    ...
    Cost:   {}""".format(
            ...
            try:
                ...
                if input_effect_id == 0:
                    if not Controller.CanDecline(is_forced, effect_descriptors):
                        # Generated input cannot be argued with: asking again would produce the
                        # same answer, so this stays a loud failure. A person can be asked again,
                        # and a client that offers Cancel on a prompt the engine will not honour
                        # should get a refusal rather than crash the game (J8).
                        assert not input_is_generated, (
                            f"Replay or skip declined a forced prompt with "
                            f"{len(effect_descriptors)} options for {message}. The recorded input "
                            f"cannot resolve it, so replaying this scene cannot continue."
                        )
                        Log.Warn(CATEGORY_NAME,
                                 f"Declined a forced prompt with {len(effect_descriptors)} "
                                 f"option(s) for {message}; asking again.")
                        return None, True
                    break

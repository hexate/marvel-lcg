...
| J38 | **The board is rendered even when nothing is displaying it, and it is 86% of the runtime.** `WorldRender.PresentInternal` walks every deck and calls `Card.Render` per card to build display descriptors, and it does so on every `Present`, regardless of what the output device does with the result. Under a headless output device that discards them, this is the whole cost of running a game. ✓ VERIFIED by profile and by experiment: `Card.Render` is called 159,813 times across three games and `PresentInternal` accounts for 6.72s of 7.77s; no-oping it runs the same eight games in 1.20s against 7.23s, a six times speedup, with round count, game-over reason and decision count identical on every seed. The engine already has the early return, at `world_render.py:59`, and the source comments there that rendering while testing is "VERY slow", but it is gated on `controller_manager.skip.is_skipping`, which also makes `Controller.ChoiceOne` overwrite the device's answer with the replay's fallthrough input (`controller.py:158-159`). So "do not draw" cannot currently be asked for without also getting "do not take my input", which is why `unit_test/harness.py` runs with skipping off and pays the full cost. The fix is to separate them: let the output device declare that it is not displaying, and return early on that rather than on the skip flag. Worth more than the speed alone, since it is the cost that makes search-based play impractical, and it would also speed the test suite | `game/world/world_render.py:45-73`, gate at `:59` | Medium, performance | **DONE** 2026-08-29. Separated the two, as suggested: `OutputDevice.IsDisplaying()` answers True by default, so a device that does not say otherwise is assumed to be showing the game; `ControllerManager.IsAnyDisplaying()` is False only when every device says it is not, and True when there are no controllers yet so setup is unaffected; `PresentInternal` returns before building descriptors on that. Placed after the render id, the prompt and the game log line rather than before them, so the only thing lost is the drawing. The skip path at `:59` is left exactly as it was. Only `unit_test/harness.py`'s `ScriptedOutput` overrides it; `WebDevice` and `ConsoleDevice` inherit True and real play is untouched. ✓ VERIFIED three ways: the whole safe suite runs in 1.56s against 5.21s and still passes 119 tests; a hundred simulated games give byte-identical results, 4 wins, 5.1 rounds and the same reason breakdown, in 5.1s against 12s; and no device other than the scripted one answers False. The simulator's monkeypatch for this is deleted, since the harness now gets it for free and so does every test built on it |
| J39 | **`requirements.txt` lists three of the seven packages the game actually imports, so a fresh install fails on the first run.** The file installs `packaging`, `Pillow` and `requests`. Its own header comment names four more it was verified against, `aiohttp`, `colorama`, `numpy` and `typing_extensions`, and two of those are hard imports on the headless path: `core/__init__.py:29` does `from typing_extensions import override` and `engine/log/log.py:4` does `from colorama import Fore, Style`. Both are unconditional, both are reached before any game starts. Found by building a clean environment on another interpreter, which is the only way to hit it: an existing dev venv already has them as transitive dependencies, so the gap is invisible on the machine that wrote the file. `aiohttp` is a genuine import too (`engine/network/`), just not on the headless path. `numpy` is correctly optional, the game falls back to its own mt19937. | `requirements.txt` | Medium, blocks a fresh install | PROPOSED |
| J40 | **A rollout mutates the live position, so every clone copies the previous rollout's graph and search cost compounds until it is unusable.** Measured over one game: successive clones took 0.048, 0.13, 0.26, ... 47.9, 110.2 seconds while the playout each wrapped stayed flat at 0.02-0.09s, so the whole cost was copying. The decisive measurement was the deepcopy memo size, which showed clones copying 28,787 objects and then 684,664 in the same game. Two distinct mechanisms, both ✓ VERIFIED by reference path. (1) Objects shared with the clone keep per-call state: `CostFunc.Discard.__init__` sets `return_original_area: Dict[CardFace, Deck]` and `on_call` rewrites it with the cards just moved (`game/ability/cost_func.py:689`); all three `Discard` instances the live world reaches are the same objects the clone reaches. (2) Worse, a live `Effect`'s `EffectContext` picks up a message created during the rollout, and messages carry a `world` back-reference, so one stale attribute drags the clone's entire `World` in: a single rollout added 228,410 newly-reachable objects to the live position. Effects and cards themselves copy correctly (0 of 75 and 0 of 81 shared), so the clone is sound; the leak is entirely the live side being written to. **DONE**: `clone.rollout_isolation(world)` snapshots both the attribute bindings and the container contents of every `game.` object reachable from the position and restores both after the rollout, on the rule that a rollout must not change the live position at all. Clone cost is now flat at 0.048/0.052/0.059s across rounds 1-3 where it used to reach 110s, and one game at variants=5 went from 238.7s to 3.3s with identical damage and identical rollout values. Search at variants=20 and 40 now adopts variants and reaches 29-30 damage where greedy stalls at 22. Four scoping mistakes are worth recording because each looked fixed and was not: scoping to `CardsDB.ability_cache` fixed only the opening position, because mid-game the retaining object is reached through a copied `Effect`; scoping to `game.ability` missed `FailureReason` and the message contexts; restoring `__dict__` alone left in-place container mutation intact, since the copy is shallow and hands back the same list; and restoring wider than `game.` put back `replay_inputs` mid-game and failed as `IndexError` in `GetReplayOperation`. | `tools/sim/clone.py`, `game/ability/cost_func.py` | High for the simulator, none for play | **DONE** |
| J41 | **`GameState` was shared with every rollout, which is the exact corruption the code that shares it exists to prevent.** `clone._isolate_gamestate` replaces synchronisation objects with fresh ones so a clone gets its own phase and step state, and its docstring says sharing it "let the rollout corrupt the game it was supposed to be predicting". The guard tested `isinstance(v, type(threading.Condition()))`, but `GameState.condition` is `engine.task.condition.Condition` (`game/game_run/game_state.py:81`), a wrapper holding a `threading.Condition`, so the check never matched, the copy raised `TypeError: cannot pickle '_thread.RLock'`, and `_best_effort_copy`'s `except Exception: new.__dict__[k] = v` shared the whole `Game.state` silently. Found only by instrumenting that fallback, which is the point: a silent share converts a hard failure into quiet corruption. **DONE**: the guard now recognises the engine's wrapper, and anything still shared is recorded in `clone.SHARE_FAILURES` instead of being swallowed (now empty for a rollout). | `tools/sim/clone.py` | Medium for the simulator, none for play | **DONE** |
| J43 | **The simulator silently plays the starter deck when given a bare hero name, and nothing in the result says which deck played.** `GameFixture` calls `SceneLoader.NewScene(scenario, None, hero_names, seed)` and `FindJsonPath('Hero', ...)` searches `DECK_FOLDERS` before `STARTER_DECK_FOLDER`, so `captain_america` resolves to `deck/starter/` while `captain_america_stun_lock` resolves to the built deck in `deck/custom/`. Both run, neither is announced. This cost a full session: every number in the 2026-08-29 sweeps was produced on starter decks while the weight files applied to them were tuned against custom decks (`weights_rhino_captain_america_stun_lock.json` is named for `deck/custom/captain_america_stun_lock.json`), and it produced a wrong conclusion, that Ant-Man losing to Rhino was a deck-building problem, when the deck under test was the starter deck rather than the built one. Starter decks are the weak case by construction, so a strategy result measured on them says little about a built deck. Fix wants the run to record the resolved deck path in its output rather than leaving the pairing to a filename convention. | `unit_test/harness.py:217`, `engine/file/manager.py:193` | Medium, invalidates results silently | PROPOSED |
...
- *Keep Python as an embedded scripting layer.* The card scripts themselves turn out to be cheap
  (E3), but that does not rescue this shape, it relocates the problem. The engine calls back into
  script-registered predicates constantly while filtering effects, so the FFI boundary would land
  in the middle of the hot loop rather than around it.
...
**Where the time actually goes (E3, measured 2026-08-29).** Self-time across four headless
Rhino / Captain America games, after J38 removed the rendering cost:

| Area | Self-time | Share |
| --- | --- | --- |
| `game/` rules engine | 1.024s | 75.2% |
| Python builtins | 0.157s | 11.5% |
| Python import machinery | 0.097s | 7.2% |
| `engine/` framework | 0.030s | 2.2% |
| `cards/` card scripts | 0.002s | **0.2%** |

This corrects the premise this section was built on. Card scripts are not the hot path, they are
0.2% of it. They mostly register `Ability` objects declaratively and return. The expensive part is
the dispatch machinery that decides which of them apply, about 4,900 message broadcasts and 3,300
condition checks per game, which is the same stack G1 measured at 87% of the replay profile. The
hot path is therefore the portable part of the codebase, and the 102k lines that make a port
expensive are the cheap part. They still cannot be separated, because the engine reaches into the
scripts through effect callables thousands of times per game.

**PyPy is measured and rejected (E4, 2026-08-29).** E2 named PyPy as the cheap in-language escape
hatch, and nothing blocks trying it: the headless path never imports numpy, no file uses syntax
newer than 3.10, the install guide already claims 3.10 support, and Pillow builds on PyPy 7.3.19
unaided. The engine runs correctly under it and produces identical games, seed for seed across 20
seeds, including the single win. It is also 2.4x slower. Best block of ten games was 0.141s per
game on CPython 3.13 against 0.340s on PyPy 3.10, and the ratio stayed flat across 120 games, so
this is not missing JIT warmup. Cause ✗ UNVERIFIED, but the shape fits: megamorphic dispatch
spread over thousands of modules with no hot loop for a tracing JIT to specialise, plus the
function-local imports that already cost 7.2% on CPython and that PyPy does not optimise away.
The direction is not marginal, so PyPy is not the escape hatch E2 hoped for.
...
**What would change this recommendation:** a genuine constant-factor wall rather than an
algorithmic one. That gate has now been tested from both sides and neither shows one. G1 landed
and found the cost concentrated in effect filtering, and E3 measures per-action cost directly at
0.15s per headless game with a flat profile and no interpreter hotspot. Meanwhile the speedup
search actually needs is roughly 100x (E5), which is more than a language change reliably
delivers and would have to be paid on top of the 102k-line rewrite, not instead of it.

| ID | Item | Status |
| --- | --- | --- |
| E1 | Decision: remain on Python; treat B1 as an algorithmic fix (snapshot/rollback) rather than a rewrite. | **STANDS**, no longer pending: G1 landed 2026-08-19 and E3/E4 close the gate from the other side |
| E2 | If a CPU wall is later confirmed, evaluate in-language escape hatches first: PyPy, and the existing `enable_multiple_threads` flag (`engine/task/manager.py`, default `False`, reason for the default is ✗ UNVERIFIED). | **PARTLY ANSWERED**: PyPy measured and rejected (E4). `enable_multiple_threads` still untried and is now the only cheap lever left in this row |
| E3 | Self-time split measured: `game/` 75.2%, card scripts 0.2%. Corrects this section's "card scripts **are** the hot path" premise, and with it the argument against a hybrid port, which now fails for a different and stronger reason. | **DONE 2026-08-29** |
| E4 | PyPy 3.10 evaluated end to end: runs the engine correctly, identical outcomes, 2.4x slower, no warmup convergence over 120 games. | **DONE 2026-08-29, REJECTED** |
| E5 | Reframe what the speed is for. Self-play tuning needs ~60,000 playouts per game and is out of reach in any language. A hint that searches one decision needs ~1,000 playouts, about 30x from today's 0.15s per game, which is plausibly reachable in CPython by hoisting the function-local imports and reusing loaded state between playouts. Size the hint, not the tuner. | PROPOSED |
...
| 2026-08-29 | **Section E's premise was wrong, and the recommendation survives anyway.** E argued against a hybrid port on the grounds that card scripts are the hot path. Measured, they are 0.2% of self-time and the `game/` rules engine is 75.2%. The scripts mostly register `Ability` objects and return; the cost is the machinery deciding which of them apply, which is the same stack G1 found at 87%. So the expensive-to-port 102k lines are the cheap-to-run ones, and the hot path is the portable part. That does not make the hybrid viable, it makes it fail differently and worse: the engine reaches into script-registered predicates thousands of times per game, so an FFI boundary would sit inside the hot loop instead of around it. E1 stands, better supported than when it was written, and is no longer pending G1. Also settled E2's PyPy half by running it rather than reasoning about it. Nothing blocked the attempt, the headless path never touches numpy, no file uses post-3.10 syntax, and Pillow builds on PyPy unaided. It runs the engine correctly and produces identical games seed for seed, including the one win in the sample, and it is 2.4x slower with the ratio flat across 120 games, so it is not a warmup artefact. Recorded as E3, E4 and E5. E5 is the part worth acting on: self-play tuning needs ~60,000 playouts a game and is unreachable in any language, while a hint that searches one decision needs ~1,000, about 30x from here, which is a CPython-sized problem. |
| 2026-08-29 | **J39 filed, found by accident.** Building a clean PyPy environment failed twice before it ran a game, once on `typing_extensions` and once on `colorama`, both unconditional imports on the headless path and neither in `requirements.txt`. The file installs three packages and its own header comment names seven. This is invisible from any existing dev venv, where all four missing ones are already present as transitive dependencies, which is why it survived a verified-install note claiming the opposite. Worth remembering as a method: the fastest way to find a broken dependency list is to build the environment somewhere the machine has never built it. |
| 2026-08-29 | **Search beats greedy, and the reason it did not before was J40 rather than the strategy.** Once the rollout stopped writing to the live position, a scaling sweep became affordable for the first time: 100 games, greedy plus variants 1, 5, 20 and 40, over the same 20 seeds. Greedy wins 0 of 20 at 20.15 mean damage against a 29-HP villain. Variants=20 wins 7 of 20 at 25.85, and beats greedy on 19 of the 20 shared seeds while losing on 1 (sign test p=0.00002, Fisher on wins p=0.0042). The curve rises steeply from 1 to 5 to 20 and then flattens, with variants=40 slightly behind variants=20 and costing twice as much, so the useful budget on this matchup is somewhere near 20. This retires the reading that the bot had hit a strategic frontier that search could not cross. It had hit a performance bug: at the budgets that help, a single game used to cost minutes, so every earlier search run was made at a budget too small to do anything, and variants=1 through 3 do indeed match greedy. **One loose end, recorded rather than resolved.** Variants=1 performs no perturbation and never adopts, so it should reproduce greedy exactly, which is the check `search.py` documents. It does not: it differs on 8 of the 20 seeds and wins 2 games greedy does not. That difference is not significant on its own (p=0.24) and does not touch the main result, since variants 5 to 40 beat the variants=1 control as decisively as they beat greedy, but it means something in the rollout still perturbs the live game and the isolation is not yet complete. |
| 2026-08-29 | **J42 opened: variants=1 still does not reproduce greedy, and five hypotheses are now ruled out.** A rollout with no perturbation never adopts anything, so it should replay greedy exactly, which is the check `search.py` documents. It differs on 8 of 20 seeds. What is established. A three-way bisect isolates the cause precisely: `greedy == stub == cloneonly` on every seed tested, where `stub` is the rollout policy with the playout removed and `cloneonly` clones and runs the isolation guard but never runs the clone's game loop. Only running `clone.OnGameLoop()` diverges, so cloning and the isolation are not at fault. Ruled out by measurement, not argument: the generator (state hash and counter identical at every decision in both runs), the whole object graph reachable from `Engine.game` (content-aware fingerprint including container contents and reference identity, only `Condition.func` differs), class-level state (only `Log.all_log_text` and `CardsDB.ability_cache`, and evicting the entries a rollout adds changes nothing), the live controller modules (`InputModule` and `UndoModule` were genuinely being reset by a rollout, now restored, still differs), and `__slots__` (nothing in the codebase uses it). Localised in game time: with the rollout's own log lines suppressed, the two runs' engine logs are identical for 245 lines and then Rhino deals 6 damage in one and 3 in the other, on the same attack, with the same boost card revealed and the same boost icons counted. **What this investigation did turn up is separate and real.** Diffing only objects present in both snapshots hid it: a rollout leaves 10,517 game objects reachable from `Engine.game` that were not before, 362 of them `Deck2`, and removes none. The path is `Game` to `ControllerManager` to the live `ScriptedDeviceManager` to the shared `SynchronizationNotifier` to a `Condition` to a closure capturing the clone's device manager, its inner policy, the `_CloneFixture` and the clone `World`. Cause is `Condition.Wait` doing `self.func = check` (`engine/task/condition.py:14`) and keeping the last waiter's closure, on a notifier that is shared with the clone deliberately because it owns three of the four locks that refuse to copy. It costs memory rather than time, because `deepcopy` stops at the shared notifier, which is why the clone memo count stayed flat while the graph was being retained. `Condition.func` is written and never read anywhere in the engine. Restoring it did not drop the retention, so that attempt was removed rather than left in on the assumption it helped, and the retention is still open. |

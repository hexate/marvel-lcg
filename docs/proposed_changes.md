...
| I6 | `WorldRender.CalculateCRC()` runs on **every** `ChoiceOne` (`controller.py:54`) and walks every card calling `GetRenderInfo()`. Harmless per decision, but it is unconditional, including during skip/replay, where nothing renders. | PROPOSED, and **measured 2026-08-19 in the G1 profile: bigger than the earlier estimate.** 152 calls at 0.117s cumulative in a 1.30s session, so 9% of runtime and 0.77ms per call rather than the ~0.1ms guessed from a microbenchmark. `GetRenderInfo` is 1974 calls at 0.099s, which is most of it. Still the same fix, skip it when nothing renders, but it is now worth doing rather than noting |

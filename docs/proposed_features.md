...
| N11 | **The start and setup pages ship browser defaults.** The two screens you meet before the game looked like neither the game nor each other. `scene.html` was almost entirely unstyled native widgets on a dark background: light grey system buttons, a system select, system file inputs, and hairline `fieldset` rectangles with the label notched into the top edge. `main.html` had the opposite problem, a copy-pasted button pack with a fixed 130x40 box, an inset white bevel, and a hover that emptied the fill and drew 2px lines round the edge instead | Q, from use: "all the buttons are ugly and square". These are the first thing anyone sees, and the board they open has had three passes of work while these had none | Small | **DONE** 2026-08-19. `public/css/menu/controls.css`, shared by both pages, restating the board's button language from N7 and N8 rather than inventing a third one: solid fill on a per-button `--color`, white bold label, 1px border at quarter-strength white, hover lightening the fill 4% with the border carrying most of the signal, `scale(.96)` on press, `:focus-visible` ring. Sized by padding now, so labels stop being clipped or swimming, and each page marks its one real action primary. Page background is the v2 felt, fixed so the vignette does not scroll through a setup page several screens tall. Two fixes beyond the brief: prose links were `rgb(0, 0, 238)` underlined, near unreadable on dark and worse visited, replaced at a checked 5.4:1 and 5.2:1; and `Join as player 2..4` are genuinely disabled, which was invisible when all four rendered as identical blue link text. ⚠ One regression found and fixed in passing, worth knowing because the obvious check misses it. `font-family: inherit` on a file input truncates it: form controls do not inherit fonts unless asked, so these had been in the browser's narrow default where "No file chosen" measures 77px and fits a 100px hero slot, against 112px in the page's monospace. `scrollWidth > clientWidth` never fires on a file input, because the ellipsis happens inside its shadow DOM, so it reports no overflow however badly the label is clipped. `measureText` against the content box is what detects it. Follow-up 2026-08-19, both of the deferred items done at Q's request. Checkboxes are drawn rather than native: they were bright white squares, the loudest thing on the page, and `accent-color` could never fix that because it only colours the checked state. `appearance: none` with a `clip-path` tick, no asset. The five panels keep their colour coding, because it is how you tell which group you are in, but the hue moved off a lit 1px outline onto one edge and the heading, with a near-neutral fill and a border matching the fieldsets. ⚠ That created a legibility regression caught on measurement: the headings became 12.8px, which wants 4.5:1 rather than 3:1, and three of five failed as raw accents against the rgb(34,37,46) panel fill (crimson 4.27, blue-grey 3.62, magenta 3.34). Each is now mixed toward white by the smallest step that clears, keeping as much hue as possible: 4.94, 4.64, 4.75. Green and amber already passed. Hence two tokens per panel, `--accent` for the edge and `--accent-text` for the heading. ⚠ Also worth knowing: `getComputedStyle().color` returns `color-mix()` results as `oklab()`, so parsing digits out of it yields nonsense rather than an error. Rasterise to a canvas and read the pixel instead. ⚠ The radio rules are written and symmetric with the checkbox ones but are **unverified**, because this page renders zero radios in any reachable state. Third pass 2026-08-19: both pages moved onto `--font-ui`, the stack N7 gave the board. They were the last things still in a fixed pitch, `menu/style.css` setting `body { font-family: monospace }` and the start page hard-coding Arial. Overridden from `controls.css` on document order rather than fixed at the source, because `menu/style.css` is shared with the deck editor, card browser and replay list, which were out of scope. `tabular-nums` came with it, since fixed digit width is the one thing the monospace face was earning: the timeout and heroic labels rewrite their own numbers as you drag them. Card ids and the serif info glyph stay fixed pitch by N7's own test. ⚠ The two font stacks are now written out in both `marvel2/type.css` and `menu/controls.css`. type.css stays the source of truth; they have to be changed together, and a shared token file would be the real fix |
| N12 | **The board has no play zones, and the width v2 won reads as emptiness.** `marvel/area.css` is 26 lines and draws nothing: no background, no border, no label. Cards sit at fixed heights with nothing saying which row is which, so a board with a few cards on it reads as an empty rectangle | Defining play zones is the first thing every write-up on card game UI asks for, and it is the cheapest structure available now the felt is done (N4) | Small | **DONE** 2026-08-19. Five lanes in `marvel2/board.css`, on the villain row and the four player rows, each a `::before` on the area positioned from the same `--y` the cards use, behind the cards and `pointer-events: none`. Band 345 to 1575, measured on a live board. ⚠ Measuring mattered: the scheme column declares `--x: 1773` but its cards are rotated a quarter turn, so their box is 176 wide not 127 and they actually occupy 1597 to 1758. A lane sized to the declared coordinate ran 112px underneath them. **The `justify-content` half was withdrawn.** `area.css` does set `display: flex` and `justify-content: center`, and both are live, but the container computes to `width: 0` with every card absolutely positioned off `--x`/`--y`, so there is no free space to distribute and no in-flow child to lay out. Editing it changes nothing. What centres a row is `move-card.ts:213`, computing the run width in JS and writing `--x` on the area. Rows therefore cannot be spread from CSS, and should not be: spreading a two-card row full width separates related cards and lengthens every mouse trip. The spare width is better spent being visibly the zone. Two follow-ups from Q the same day. ⚠ The first lane overlapped its neighbours by exactly 6 units: rows are on a 190 pitch (176 card plus N5's even 14 gap) and the lane added 10 units of air above and below, taking 20 out of a 14 gap. There is no room for padding here at all, and the gap matters more than air inside a lane because separation is what makes zones read as separate. The lane is the card's band exactly now, leaving a measured 10px between every pair. Second, the lanes are coloured by side: villain and engaged minions warm, allies/supports/hero cool. Engaged minions count as enemy because a minion engaged with you is what you are deciding whether to attack. The warm side carries the signal and the cool side stays near neutral on purpose, since `.player-background` already tints player seats per seat (blue at 0, red at 1, green at 3) and a strongly coloured player lane would agree with one seat and fight the other three |
| N10 | **An effect whose size comes from a quantity you choose lets you choose zero, get nothing, and be told nothing.** Two cards hit this from opposite directions, which is why it is one row rather than two. **(a) The quantity is a discard, and the cost step is noise.** Shield Toss (`03006`) is "Discard X cards from your hand, then return Captain America's Shield → deal 4 damage to X enemies". The script takes X from the discard and deals damage to `effect.targets[:discard_num]`, so selecting three enemies and discarding none resolves to nothing at all. **(b) The quantity IS the cost, and the step is hidden.** The engine models X properly as `Cost(..., up_to=True)`, documented in `game/element/cost.py:46` as "Can spend 0 ~ rbyga", and six cards use it. Machine Man (`cards/pack/vision/26022.py`) is `Cost("3", up_to=True)` reading `effect.GetPaidResources().val` for attack and thwart. But `select.ts:78` reads `if( !need_cost || getCost() == "" || isCostRuleUpTo() ) { ok = true }`, so an `up_to` cost short-circuits the whole cost branch. Everything in the `else` is skipped, and that `else` is where the running total lives (the `Cost [card] (paid) / (cost)` prompt) and where `over_pay` is set. So an X-cost card shows no running total, its button reads "Cost" rather than "OverPay", and OK is live from the start | Reported from play on (a): three minions selected, three resources paid, the confirm button read "overpay", it resolved and every enemy was untouched. The rules are right, the flow is not. On (a) three things mislead: the card costs 0, so the resource prompt is irrelevant yet is the most prominent step, and "overpay" reads like buying more effect; the discard range starts at 0, so doing nothing is a valid answer to the step that actually matters; and target selection is never reconciled with the discard. (b) fails the opposite way. Shield Toss shows you a cost step that does not matter; an X-cost card hides the one that does. Same root either way: the size of the effect comes from a quantity the player picks, zero is legal, and nothing on screen says what you are about to buy | (b) is small and mostly deletion. (a) is small for a warning, medium to bind the discard and the targets together | **DONE** 2026-08-19, both halves. `up_to` now skips the validation without skipping the display: handled inside the cost branch and forced true after the type rules, so none can put it back. Label reads "Pay up to" rather than "Cost", since the number is a ceiling not a price, and an empty selection renders "0" rather than a blank bracket. `over_pay` left alone deliberately, because on an X cost it means past the cap, which buys nothing. ⚠ Verified at runtime the awkward way: none of the six `up_to` cards appears in any replay in the corpus, so no recorded game reaches this code. Imported the live modules in the board page and drove `SelectStep.setCost` against a stubbed effect. X cost 3 shows "Pay up to (0) / (3)" with OK enabled at zero, a running icon total as resources are added, and OverPay only past the cap; ordinary costs still block at 2 of 3 and confirm at 3. **(a) attempted 2026-08-19 and backed out, unverified rather than failed.** What the attempt established, which is most of the value: **The order is fixed.** `SetTarget` is resolved before `Play` pays the costs (`card_player.py:126`), so a callable target range cannot read the discard. **`SetTarget2` is not what its comment says.** `ability.py:61` claims the other selectors are "only use the first one, the others are used to check if it has the legal target", but `effect_context.py:59` shows `targets2` calling `AskChooseSelect(..., for_second_target=True)`. It is a real interactive selection, made lazily on first access from inside the ability body, which is AFTER `ProcessSelfCost`. **Cost ranges take lambdas.** `CostFunc.Discard(..., range=(lambda effect: ..., ...))`, as `spdr/31007.py` does. So the fix looks like: keep the discard as the cost, move the enemies to `SetTarget2` with `range=(discarded_num, discarded_num)`, and the two can no longer disagree. ⚠ Written, and backed out because it could not be verified. Built a probe on the I2 harness (`GameFixture("rhino", ["captain_america"])`, which works and puts the Shield in play at setup), but the probe deals 0 damage for the ORIGINAL card as well as the changed one, so it cannot tell them apart and proves nothing either way. The `Play()` cheat appears not to run the primary target selection. Shipping a card restructure on that evidence risks silently making Shield Toss deal 0 damage always, which is worse than the current bug. ⚠ Also worth knowing: no replay in the corpus contains Shield Toss or any Captain America game, so there is no recorded coverage of this card at all. **Probe built 2026-08-19: `tools/probe_shield_toss.py`.** It answers the question it was built for, in the negative, and that is the useful part. The discard plumbing works: answering `Pay_cost_Discard` with N cards drops the hand by exactly N, at N = 0, 1 and 2. The primary target selection never prompts and does not need to, because `UpdateLegalTargets` gives `all_legal_targets = [Rhino]` and `target_range = (1, 1)` (the max is capped by the number of legal faces), and a forced single choice is taken silently. ⚠ But `DealDamage` is **never called** and `effect.targets` is empty after the play, while the costs plainly do run, since the hand shrinks. So **`Play()` in the cheat DSL pays a card's costs without executing its ability.** Any test that measures a card's effect through that cheat measures nothing, and reports 0 for a working card and a broken one alike. That is exactly what made the first N10a attempt unfalsifiable, and it would have done the same to any retry. **(a) DONE 2026-08-19.** The enemies now come from `targets2`, asked lazily from inside the body and therefore after the discard, with the range bound to it, so a target you cannot pay for is never offered. ⚠ Two non-guessable things had to be understood first. `SetTarget2`'s range callable cannot see the effect that owns it: `AskChooseSelect` (`player_ask.py:206-218`) builds a throwaway ability via `ForChoiceAbility(...).SetTargetInternal(selector)` and asks THAT, so the selector is attached to a fresh effect with an empty `cost_func` list. Logged from inside the callable: body on effect object 21 with `['Discard','ReturnToHand']`, range callable handed effect object 1 with none. A second-target range can therefore only read world state, which is why the one prior example reads the villain's stage. The count is carried in a cell written at the top of the body. ⚠ And the range is `(0, n)` not `(n, n)`: `GetTargetRange` returns None outright when the minimum exceeds the enemies available (`selector.py:83`) rather than clamping, so `(2,2)` against a lone villain deals 0, the original bug in a different hat. Verified end to end: discard 0 offers no enemy and deals 0; discard 1, 2 and 3 each deal 4. Suite green. **Turn-flow probe (`tools/probe_shield_toss.py`) reproduces the original bug.** `tools/probe_shield_toss.py` drives the real `WhenPlayerInTurn` ask: discard 0 gives `DealDamage(targets=[], amount=4)` and 0 damage, while discard 1 gives `DealDamage(targets=[Rhino])` and 4. At discard 0 the card still FORCES a target, `target_range=[1,1]` on Rhino, so the player picks an enemy, the card resolves, and nothing happens. That is the report, reproduced and measurable. ⚠ Two things the probe had to get right, both of which silently faked a pass before: Shield Toss needs Captain America's Shield IN PLAY, and at setup the Shield is in hand (the log line "were placed on table" is the deck search putting it there), so the Shield must be played first or Shield Toss is never a legal option; and the policy must ANSWER selection prompts rather than decline them. ⚠ **The `SetTarget2` fix is disproven, by measurement rather than reading.** Two problems. A selector range is evaluated whenever legal targets are recomputed, which happens while building the turn's action list long before any cost is paid, so `cost_func.Get(CostFunc.Discard)` asserts and crashes the game just deciding whether the card is playable (`Has` avoids that). And with that fixed `targets2` is never asked at all: the play option reports `target_range=[0,0]`, no enemy prompt is raised even with a policy that answers everything, and `DealDamage` gets empty targets at every discard count. So it makes Shield Toss deal 0 damage always, worse than the bug. Reverted. The next attempt needs a different mechanism, and should start by finding out why `targets2` is not reached here, given `effect_context.py:59` does call `AskChooseSelect` for it |
| N13 | **The setup screen will not list the decks you have.** Choosing a hero was four file inputs. You opened a file dialog and had to already know decks live in `./deck/starter`, which is why the paragraph under them said so. Both listing routes existed and neither was used by this page: `list_starter_deck` over `./deck/starter`, and `list_deck` over `deck_folders`, which `launch.json` sets to `./deck/` and `./deck/custom/`. Neither was a name you could see, and `./deck/` itself holds no json, so the second route is the custom folder in practice | Q, from use. The set half of this screen has been a grid of thumbnails since it was written and the hero half was a file dialog. Keeping starter and custom apart is the point rather than a nicety: a starter deck is the precon and there is one per hero, a custom deck is a built one and several can exist for the same hero | Small to medium | **DONE** 2026-08-20. `public/scene.html`, in the Heroes fieldset, plus the two rollover and panel changes below in `public/css/menu/set-detail.css`. Two `<details>` groups, 63 starter and 24 custom, drawn as the same 100x140 portraits as the rest of the screen. Clicking one loads it into the first free slot and marks it, clicking it again takes it back out. The four file inputs stay, because a deck from anywhere on disk still has to be loadable. `SetHero` is the single funnel every route into a slot already went through, so it is what keeps the picker's marks in step: it takes an optional source now, and the file inputs and the clipboard import pass none, which clears the mark. Verified by driving it: click fills slot 1, second click empties it, a file-input load into slot 2 leaves slot 1 alone, and `heroes[1]` holds the same parsed object the file path produces. Names are on by default with a toggle, which is not decoration: every Iron Man deck carries the same identity art and there are three of them in `deck/custom`, so the portrait alone cannot tell them apart. Two things Q asked for on top, both about the same gap. **Names on rollover.** The hover preview shows the card a thumbnail stands for, and for a set that is the set's FIRST CARD, so the picture could never say which set you were pointing at; the name only appeared inside the detail panel you had to open. The panel now carries a caption, fed by a `data-hover-name` attribute set on encounter and modular sets, on scenarios (expert distinguished), and on deck portraits. ⚠ The preview is keyed on the card plus the name rather than the card alone, or moving between two decks built on the same hero is a no-op and the caption keeps naming the deck you have left. **An `i` badge on a deck**, the encounter sets' badge and its rule: hover to reveal, `stopPropagation`, so reading a deck can never change your setup. `SetDetail.show` takes named groups instead of one flat list, so a deck reads as Identity / Signature cards / Set aside / Player deck / Side deck / Obligation / Nemesis set with per-section counts, rather than 47 cards in a heap. An encounter set passes one untitled group and draws exactly as before, checked. Side deck is included because the deck editor writes the key, though none of the 87 shipped decks has one. ⚠ **Two bugs found on the way, both invisible until something was anchored next to them.** The rotated card in the hover preview was painted 128px off its own box: `margin: 64px -64px` centres a 320x448 image at (96, 288) of a 448x320 box, and it wants `-64px 64px`. Nothing showed that while the panel was only ever the card, because the box was invisible and a card drawn outside it still looks like a card. The caption is anchored to the box, so the strayed card painted straight over it, and a rotation is a stacking context, which is why it won. Re-measured after: the painted card now fills its box exactly. And `describeComposition` lowercased card types without splitting the camel case, so a deck summary read "1 playersidescheme". Split first, and added plural labels for the player types, or every deck panel would have said "9 allys" and "1 heros". ⚠ Worth knowing for anything else drawn on this page: `menu/style.css` puts `white-space: nowrap` in reach of these buttons, so the first version of the deck name was one clipped line inside the card art. It sits under the portrait now, two lines tall whether the name needs one or four, so the rows stay level and the art stays visible. ⚠ And the picker costs one `get_hero_json` per deck on load, 87 of them, because the listing routes return file names only and the portrait, the deck's own name and its size all come out of the file. They go out together rather than in a loop, which is what `deck.html` does |

| N14 | **A facedown encounter card dealt to you lands in your hero row, under your cards in play.** Villain phase step 3 deals one card per player into `player.dealt_encounter_cards` (`world.py:426`, `player_action.py:35`) and it sits there facedown until step 4 reveals it. The client put that zone in `player-all-area-hero`, the shared row holding every player's hero, upgrades, obligations and environment | Q, from play: it arrives immediately after you defend, so it reads as a consequence of defending rather than as the next step of the phase, and stacked among your cards in play it reads as something entering play rather than something waiting to be revealed. The same failure as N2 one step later in the same phase, two events with opposite consequences drawn in the same place | Small | **DONE** 2026-08-20. The zone has its own container per player now, `player-N-dealt-encounter-cards`, sitting in the left pile column with that player's deck and discard. **Most of it was already written and had never been connected.** `pos-deck.css:18-23` positions `#player-all-dealt-encounter-cards` and `.dealt-encounter-cards.player-0..3` at `--x: 20`, `--y: 570`; `player.css:35,45,49` styles `.area.dealt-encounter-cards`; `game.css:98` and `card-game-area.css:7-22` style the cards inside it; `cards.ts:156` maps an id ending in `dealt-encounter-cards` to the `'dealt'` area type, which is the branch that skips sorting; `move-card.ts:48` has a `parent.classList.contains('dealt-encounter-cards')` lane that fans the cards half a card apart; and `card_animation.ts:244` has the deal animation. The id appears in exactly one file in the repository, the stylesheet, and has since `47e9866`, the open source initial commit. So this was a container that was designed and never built, or removed before release. What was missing was the DOM node and two lines of routing: the four divs in `marvel.html` and `marvel2.html`, the ids in `Cards.area_names`, the push in `printCardsObjs` moved off `player-all-area-hero`, and `descriptor.ts:433` giving each card `player-${control_player}-dealt-encounter-cards` as its parent. `isShowDetailArea` gains the suffix as well, or a card being revealed loses its stat badges and the weather hook, since `RevealEncounterCards` (`player_action.py:73`) flips the card while it is still in this zone. **A row of its own in the centre column was never available**, which is the same wall N2 option 7 hit and the reason this is a container rather than an offset. Rows are on a 190 pitch against a 176 card, 14 units of gap, and 4 at the bottom: half a card below the hero row is inside your hand. The left column had the space because the stylesheet had been holding it open. Per player rather than one merged container, matching the decks: everything carrying a `.player-N` class is `display: none` unless that seat is focused (`player.css:15-20`, driven by `UI.focusOnPlayer` at `ui.ts:452-466`), so in multiplayer you see your own dealt card and not three others fanned into the same 90px slot, which is what a merged container at a single `--x` would have given. That is a behaviour change from the shared hero row and is deliberate. ✓ VERIFIED by driving the Black Panther replay and watching a card fly from the encounter deck and settle, in all three renderings. Viewport top and bottom, same column throughout: v1 encounter discard 71-197, dealt card **407-533**, player discard 550-675, player deck 692-818; v2 flat 88-206, **407-533**, 550-677, 694-824; v2 3D 88-206, **408-532**, 550-677, 694-824. No overlap anywhere, and the 210 units above the card is the empty band the stylesheet reserved. Also counted zero dealt cards reaching the hero row across the whole replay. ⚠ `.image-preview` spans 91-433 across and 102-577 down, so it covers about 15px of the dealt card's right edge while a preview is up. Pre-existing rather than introduced: it already clips the top corner of the player discard pile in the same column. ⚠ Two guards are now unreachable and were left alone rather than removed: `is_dealt_card` in `sortFn` (`cards.ts:280`) and `!card.is_dealt_card` in `buildAreaXList` (`move-card.ts:129`). Both are in the `'area'` path, and a dealt card no longer enters an area that runs it. ⚠ Worth knowing for anything else verified this way: a replay cannot be paused from `#replay-btn`, which does not exist on the board page, and the deal animation is 2500ms, so a screenshot loses the race. Cloning the settled card into a fixed-position copy is what holds it still, and the clone must keep its inline style, because the art is in `--bg-image-false` and overwriting `cssText` leaves an empty box |

| N15 | **The board's motion is either not eased or eased in one direction only.** Three of them, one theme. (a) `card.css:78-81` transitions `transform`, `opacity` and `scale` `linear` over `--anime-move-time`, 0.3s. `.card` is positioned by `translate3d(var(--left), var(--top), var(--t-z))` at `card.css:68-73`, so that one rule carries every draw, play, discard and row change on the board. (b) `card.css:103-105` sets `transition: none` on a hovered card's children, so the hover lift snapped in and eased out. (c) `side-bar.css:46-52` does the same to the two drawers, which have `.15s linear` at `:16` and `:22`: the transition only ever applied to the return, so they slammed open and slid shut | Constant velocity ending in a dead stop is the clearest tell that a movement was never given a curve, because nothing with mass moves that way, and it is on the motion you see most. The two asymmetries are worse than either extreme would be: in-fast-out-slow reads as the interface being twitchy rather than quick. Q asked what would make the game feel more modern and smoother, and this was the largest answer that is purely presentational | Small, one new stylesheet | **DONE** 2026-08-20, `public/css/marvel2/motion.css`, linked last in `marvel2.html` because section 2 wins on source order rather than specificity. Position moves on `easeOutCubic`. The project's existing `cubic-bezier(0.19, 1, 0.22, 1)`, `easeOutExpo`, was deliberately not reused for it: sampled over 0.3s that curve is 69.5% travelled at 50ms and 91.8% at 100ms, so a card arrives before you have registered it leaving and then spends 200ms settling, which reads as a teleport rather than as travel. `easeOutCubic` over the same 0.3s is 43.7% at 50ms, 72.2% at 100ms and 95.4% at 200ms. `easeOutExpo` is kept for `scale`, where the whole distance is 15% of a card and arriving fast is the point. `opacity` stays `linear`: a fade has no position and no momentum, so there is nothing for an ease to model. No duration changed anywhere in the file, so the two layouts stay comparable on the same timings. The hover asymmetry is settled by specificity rather than by editing v1: an id outranks any number of classes, so `#scene .card .face .image` at (1,3,0) beats `.card:hover .face .image` at (0,4,0). The `animation: none` in that same block is untouched, so card effects still hold still under the cursor, which is what it was there for. ✓ VERIFIED on a live hovered card in a replay: `--this-scale` 1.15 applied, `transition-property: scale, background-color, box-shadow` surviving the hover, `animation-name: none` still holding, and `transition-property: right` on the side bar while `:hover` matches, where it computed `none` before. Deliberately not touched: `.card:hover .face`, which the same block also freezes, costing the `rotate` transition on a card flipping while hovered. Rarer event, separate argument. J30 was found while doing this and is the larger half of what was actually wrong |

| N16 | **Reduced motion is unreachable, and the escape hatch that exists has never run.** `settings.ts:63` loads `./public/css/disable-animation.css` on `?disable_animations`. The file is at `./public/css/marvel/disable-animation.css`. `loadCSS` assigns `href` verbatim, so it 404'd and the parameter only ever set `no_tilt`. Separately, `prefers-reduced-motion` appeared in no stylesheet in the project, so the operating system setting was never consulted at all | Third instance of this exact shape after J28's `./css./marvel./scene-3d.css`, which is the argument for it being a pattern rather than a typo: `loadCSS` takes a string, never validates it, and a 404 on a stylesheet is silent in every browser. Someone who has asked their machine for less motion had no way to be heard short of knowing about an undocumented query parameter that did not work | Small | **DONE** 2026-08-20. Path fixed in `settings.ts`; `motion.css` section 4 adds the media query, using the same two properties and the same `!important` that `disable-animation.css` uses, on purpose, so there are not two subtly different definitions of what "off" means. ⚠ Fixing the path enables a stylesheet that had never once executed, so it was tested rather than assumed: on a freshly loaded replay with the flag on, 80 of 81 cards are placed, the hero row is populated and the board renders correctly. Worth recording how nearly this went the other way. The first test appeared to show the flag breaking the board completely, hand and hero row missing, cards piled at the origin with `--x` unset. That was a measurement artifact: reloading the page against an already-loaded scene does that on its own, and it reproduces identically under `?v1`, which loads none of this. The control run is what caught it. Honest limit: the spec asks for reduced motion, not for none, and this removes all of it. That is a deliberate match to the existing switch rather than a judgement that none is right, and it is the safer of the two for anyone whose reason for setting it is vestibular. v2 only, like everything else in that directory; linking the file from `marvel.html` is all it would take to change that |

| N17 | **Every statistics toast is headed with the word "STATISTIC".** `notify.ts:83` passes that literal string as the title to `Notify.create`, so a toast whose body already reads "<card art> Enters Play x10" carries a header naming the category of the message rather than saying anything about the event | Reported from play by Q: "that text is not necessary". These fire constantly, once per card entering play at the default `statistics_first` setting, so it is the most repeated piece of text in the game and it is the piece carrying the least. The body is self-describing: art, what happened, how many | Small | **DONE** 2026-08-20. The title is an empty string now. ⚠ That alone would have made it worse rather than better, which is the only interesting part of this. The vendored `notifications.js` builds a `.notititle` unconditionally and `notification.css:93-100` gives it `0.65rem 0.5rem 0.4rem 1.25rem`, so an empty title leaves roughly 1.05rem of dead space instead of closing up, and `.notidesc` carries no top padding of its own because it assumes the title above supplies the gap, so the message would then have sat flush against the card's top edge. Two rules in `notification.css` cover both: `.notititle:empty` collapses, and `.noticard:has(> .notititle:empty) > .notidesc` borrows the title's top padding. Both only match an empty title, so every toast that has one is untouched. ✓ VERIFIED on a live replay: title `""` computing `display: none` at height 0, description at `padding-top: 10.4px`, body reading "Enters Play x10". Note this is `public/css/marvel/notification.css`, shared with v1, rather than a `marvel2/` override. The title string lives in shared JS so the empty title reaches both layouts, and scoping the CSS to v2 would have left v1 holding the blank strip. Deliberately left alone: the `COMMAND` title on `showCommand` and the `RESPONSE` title on `showResponse`, which is debug level. Q named this one, and those two are rarer and closer to earning their header |
...
| Date | Change |
| --- | --- |
| 2026-08-20 | Added N14 and did it, from a play report: "an encounter card is drawn and then it is moved to under my hero in the hero row, why?". The engine was right and the question was about the drawing, which is the useful shape of this report. Two things worth carrying forward. The first is that the client already contained a complete, disconnected implementation of the fix: position, styling, sort behaviour, layout lane and animation all present since the open source commit, with only the DOM node missing. Grepping an id across the whole repository before designing anything is what found it, and it turned a layout decision into four lines of routing. The second is the shape of the answer, which is N2's again. There is no free band in the centre column and there will not be one, so anything that needs to be visibly apart from the board rows either takes an offset inside its own area or moves to the pile column, and the pile column is the one with room. |
